...
function toRecord(row: Record<string, unknown>): CognitiveTraceRecord {
  return {
    ...
    contradictions: (row.contradictions as CognitiveTraceInput["contradictions"]) ?? null,
    externalIntelligence: (row.external_intelligence as CognitiveTraceInput["externalIntelligence"]) ?? null,
    externalIntelligenceAt: (row.external_intelligence_at as string | null) ?? null,
...
export async function persistCognitiveTrace(input: CognitiveTraceInput): Promise<PersistCognitiveTraceResult> {
  ...
  const { data, error } = await learningDb
    ...
    .insert({
      ...
      external_intelligence: input.externalIntelligence,
      external_intelligence_at: input.externalIntelligenceAt,
