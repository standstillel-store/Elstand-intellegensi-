...
// (`status === "VALID"`) learning constraint currently exists for
// ELVOID_PRO_ORACLE. This route triggers NOTHING — it never calls
// `runAutonomousCycle`/`runAutonomousBatch`; it only reads what the
// runtime has already produced, so polling it on a page view can never
// itself cause a Paper Trade.
// ---------------------------------------------------------------------------
...
export async function GET(req: Request) {
  ...
  const [traces, validations] = await Promise.all([listDecisionTracesBySymbol(symbol, undefined, 1), getConstraintValidations("ELVOID_PRO_ORACLE")]);
