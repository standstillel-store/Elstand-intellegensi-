...
## Phase 8.2.7 — Autonomous Paper Execution Adapter (pre-existing in this codebase snapshot; no CHANGES.md entry was found for it prior to this pass)

`lib/ai/autonomousExecution/{contracts.ts,execute.ts}` and `scripts/phase8/autonomous-paper-execution-fixtures.ts` already existed, complete and untouched, when Phase 8.2.8 began — confirmed by direct file inspection, not assumed. `executeAutonomousPaperTrade()` gates on `input.decision.decision === "EXECUTE"` (WAIT/REJECT short-circuit before any execution call), then dispatches `source === "ELVOID_PRO_ORACLE"` verbatim into the pre-existing `lib/ai/oracle/execute.ts::executeOracleSignal()` → `lib/elvoid/paperTrader.ts::executeSignal()` path. It has zero callers anywhere in the app (confirmed by `grep -rl executeAutonomousPaperTrade`) — infrastructure only, same "unwired" convention as every other 8.2.x phase. Documented here retroactively, this pass, purely so Phase 8.2.8's own entry below has an accurate "as-found" baseline to reference.

## Phase 8.2.8 — Closed Learning Feedback Loop

**Task**: wire successful autonomous `ELVOID_PRO_ORACLE` paper executions into the existing Phase 8.1.x Learning DB pipeline (Autonomous EXECUTE → Paper Trade → Trade Outcome → Decision Experience → Decision Evaluation → Failure Pattern → Adaptive Constraint → Constraint Validation → future decision context), reusing — never duplicating — 8.1.x logic, orchestration/integration only.

**Finding, from direct inspection (not assumed)**: the outcome/evaluation half of this loop was **already fully wired and already reachable from Phase 8.2.7**, contributed entirely by two pre-existing, unmodified functions that predate this pass:
- `lib/ai/oracle/execute.ts::executeOracleSignal()` already calls `captureDecisionExperienceBestEffort()` (Phase 8.1.0) on every successful execution path — and Phase 8.2.7's `executeAutonomousPaperTrade()` reaches this by calling `executeOracleSignal()` directly, verbatim, with no wrapper of its own.
- `lib/elvoid/paperTrader.ts::writeClose()` already calls `completeDecisionLearningLifecycle(signal.id).catch(...)` (Phase 8.1.1.1) for **every** signal that closes, regardless of source — this is what a `ELVOID_PRO_ORACLE` paper trade opened via 8.2.7 eventually runs through once `evaluateOpenTrades()`/`closeSignalManually()` closes it. Source isolation falls out naturally: only `ELVOID_PRO_ORACLE` signals ever got a `decision_experiences` row in the first place (nothing else calls `captureDecisionExperience()`), so `captureAndPersistOutcome()`/evaluation is a documented no-op for every other signal, never a fabricated outcome.
- Idempotency/duplicate-work protection for this whole chain already lives at the DB layer, unmodified: `decision_experiences` uses `upsert(..., ignoreDuplicates: true)` on `UNIQUE(source_signal_id)`; the outcome patch is an `UPDATE ... WHERE outcome_result IS NULL` at-most-once guard; `decision_evaluations` uses the same `ignoreDuplicates` pattern. None of this needed to be added or touched.

Net result: **zero changes to any protected file were required** to close EXECUTE → Paper Trade → Trade Outcome → Decision Experience → Decision Evaluation. `lib/ai/oracle/execute.ts`, `lib/elvoid/paperTrader.ts`, `lib/ai/decisionOutcome/*`, `lib/ai/decisionEvaluation/*`, `lib/ai/decisionLearning/lifecycle.ts`, and `lib/ai/autonomousExecution/*` are all untouched by this pass.

**Failure Pattern (8.1.2) / Adaptive Constraint (8.1.4) / Constraint Validation (8.1.5) were deliberately left exactly as-is** — each is a recompute-and-upsert aggregate over the whole historical population (not a per-decision event), and each module's own header already documents that wiring an automatic trigger is a future, separately-approved step. Per this task's own explicit instruction, this pass does not force these to run synchronously after every trade close — doing so would contradict their aggregate-recompute design. `recomputeFailurePatterns()`/`recomputeAdaptiveConstraints()`/`recomputeConstraintValidations()` remain independently callable, unwired, exactly as found.

**"Future decision context" reachability** already exists structurally as of Phase 8.2.0: `buildAutonomousDecisionContext()` already accepts a `DecisionMemoryResult` and a `ConstraintValidation[]` and folds them into `AutonomousDecisionContext.memory`/`.validConstraints`. Supplying real memory/constraint values to that assembler is decision-context wiring, not learning-feedback wiring, and stays out of this phase's authority boundary (8.1.x remain the sole learning authorities; this phase never reads/writes `decision_memory`/`adaptive_constraints`/`constraint_validations`).

### What this pass adds

- `lib/ai/autonomousLearning/contracts.ts` — closed types only: `AutonomousLearningLifecycleStatus` (`LIFECYCLE_REACHABLE`/`SKIPPED_WAIT`/`SKIPPED_REJECT`/`SKIPPED_UNSUPPORTED_SOURCE`/`SKIPPED_EXECUTION_FAILED`), `ClosedLearningFeedbackLoopResult` (`symbol`/`status`/`willEnterLearningLifecycleOnClose`/`sourceSignalId`), and `CLOSED_LEARNING_LOOP_STAGES` (a 9-stage documentation-only array — never a state machine, never drives branching). Header documents the full discovered call chain above.
- `lib/ai/autonomousLearning/lifecycle.ts` — `classifyAutonomousLearningLifecycle(execution: AutonomousPaperExecutionResult): ClosedLearningFeedbackLoopResult`. Pure, synchronous, zero I/O. Classifies an **already-produced** Phase 8.2.7 result into whether the (already-existing) outcome/evaluation lifecycle is guaranteed to be entered once the resulting position closes. Reuses `execution.paperTradeId` verbatim as `sourceSignalId` — the exact identifier `writeClose()` already keys `completeDecisionLearningLifecycle()` off of; never a second/invented id. Does not call `executeAutonomousPaperTrade()`, `executeOracleSignal()`, `completeDecisionLearningLifecycle()`, or any DB function — it reports what the existing wiring guarantees, it never triggers anything itself.
- `scripts/phase8/autonomous-learning-fixtures.ts` — 17 fixtures (see below).

This is intentionally a thin, additive **integration adapter** in the sense the task requested — not a new orchestrator that re-sequences or re-triggers the 8.1.x pipeline (there was nothing left to sequence), but a single honest classification any future caller (dashboard/monitoring/ops tooling) can use to answer "did this autonomous decision's trade enter the learning pipeline?" without re-deriving the WAIT/REJECT/unsupported-source/execution-failed rules ad hoc.

### Fixtures — `scripts/phase8/autonomous-learning-fixtures.ts` (17/17)
...
2. WAIT → `SKIPPED_WAIT`, `willEnterLearningLifecycleOnClose: false`, `sourceSignalId: null` — zero learning lifecycle reachability.
3. REJECT → `SKIPPED_REJECT`, same zero-reachability shape.
4. Failed paper execution (`EXECUTION_FAILED`) → `SKIPPED_EXECUTION_FAILED`, `sourceSignalId: null` — no fake outcome ever fabricated.
5. Unsupported source (`SKIPPED_UNSUPPORTED_SOURCE`) → classified as such, never `LIFECYCLE_REACHABLE` — source isolation.
6. Defensive fallback: `outcome === "EXECUTED"` but `paperTradeId === null` (an upstream-invariant violation this function never simply trusts) → falls back to `SKIPPED_EXECUTION_FAILED`, never `LIFECYCLE_REACHABLE`.
7. Determinism — repeated calls on the same input produce byte-identical (`JSON.stringify`-equal) output.
8. Input immutability — the `execution` argument is unchanged after classification.
9. Duplicate classification calls produce identical, side-effect-free results (this module performs no I/O at all, verified separately by fixture 12 — so "calling it twice" is definitionally not a duplicate side effect).
10. All five `AutonomousLearningLifecycleStatus` values are independently reachable.
11. Static scan — `autonomousLearning/*` imports none of: `decisionOutcome/repository`, `decisionEvaluation/{evaluate,repository}`, `failurePatterns/*`, `adaptiveConstraint/*`, `learningValidation/*`, `decisionLearning/lifecycle`, `oracle/{grading,execute}`, `elvoid/{engine,scanners,paperTrader}`, `lib/supabase`, or `@supabase/supabase-js`.
12. Static scan — `lifecycle.ts` contains no `.from(`, no `await`, no `fetch(` — genuinely pure/synchronous.
13. Static scan — no `Date.now()`/`Math.random()`.
14. Regression guard — the two pre-existing call sites this phase's documentation relies on are still actually present, verbatim, in the untouched protected files: (a) `paperTrader.ts::writeClose()` still calls `completeDecisionLearningLifecycle(signal.id).catch(...)`; (b) `oracle/execute.ts::executeOracleSignal()` still calls `captureDecisionExperienceBestEffort(row, learningContext)` on every success path.
15. Regression guard — `decisionOutcome/repository.ts` still guards duplicate work via `ignoreDuplicates: true` upsert and an `outcome_result IS NULL` conditional update.
16. No canonical Oracle/paper-trade field (`grade:`/`stopLoss:`/`takeProfit:`/`riskStatus:`) is assigned anywhere in `autonomousLearning/*`.

**Fixture-script bug found and fixed during this pass (not a source-code issue)**: an early draft of this fixture script's `stripComments()` helper stripped block comments (`/* ... */`) before line comments. Several files in this repo write glob-style path references as prose inside `//` line comments (e.g. "`lib/ai/decisionOutcome/*`") — a block-comment-first stripper misreads that literal `/*` substring as a real comment opener and over-consumes real code up to the next `*/` anywhere later in the file, silently emptying most of the file. Fixtures 11–13 and 16 were passing against a nearly-empty stripped string, not a real absence of the checked pattern. Fixed by stripping `//` line comments first, then `/* */` blocks (confirmed by re-diffing stripped output length before/after: `contracts.ts` went from 808 chars — wrong — to 1171 chars of real preserved code). Fixture 14/15's regression checks against `paperTrader.ts`/`oracle/execute.ts`/`decisionOutcome/repository.ts` deliberately use the **raw**, non-stripped source instead (those files' own header comments use the same "`path/*`" prose convention), since the checks only need to confirm a real code string is present, not exclude it from commented-out code.

### Full relevant Phase 8 regression suite — run this pass, all pre-existing suites still pass, zero regressions

`cognitive-observation-fixtures.ts` (8.0.1), `cognitive-memory-fixtures.ts` (8.0.2), `cognitive-hypothesis-fixtures.ts` (8.0.3), `cognitive-conflict-fixtures.ts` (8.0.4), `cognitive-context-fixtures.ts` (8.0.5), `decision-outcome-fixtures.ts` (8.1.0, 33/33), `learning-db-env-fixtures.ts` (8.1.0), `decision-evaluation-fixtures.ts` (8.1.1, 36/36), `decision-learning-lifecycle-fixtures.ts` (8.1.1.1, 21/21), `failure-pattern-fixtures.ts` (8.1.2, 16/16), `decision-memory-fixtures.ts` (8.1.3, 20/20), `adaptive-constraint-fixtures.ts` (8.1.4, 21/21), `learning-validation-fixtures.ts` (8.1.5, 27/27), `decision-trace-fixtures.ts` (8.2.1, 20/20), `decision-qualification-fixtures.ts` (8.2.2, 23/23), `macro-intelligence-fixtures.ts` (8.2.3, 43/43), `event-impact-fixtures.ts` (8.2.4, 51/51), `pre-entry-validation-fixtures.ts` (8.2.5, 25/25), `autonomous-decision-fixtures.ts` (8.2.6, 25/25). Every one of these was re-run directly this pass, not assumed; none of their source files were modified.

**One suite could not be executed in this sandbox — pre-existing environment limitation, not a regression**: `autonomous-paper-execution-fixtures.ts` (8.2.7) fails to even start with `Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@supabase/supabase-js' imported from lib/supabase.ts`. That fixture script imports `executeAutonomousPaperTrade`, which transitively imports `lib/ai/oracle/execute.ts`, which imports `lib/supabase.ts` — a real (non-type-only) runtime import that resolves `@supabase/supabase-js`, a package that isn't installed in this sandbox (no `node_modules`, matching every earlier phase's own documented `npm install`/network-egress limitation). `lib/ai/autonomousLearning/*` deliberately has no such import (fixture 11 statically confirms it never imports `lib/supabase`/`@supabase/supabase-js`/`oracle/execute` at all), so this pass's own new fixture suite is unaffected and runs cleanly offline.

**One pre-existing fixture assertion remains expected-stale, unrelated to this pass and NOT a new regression**: `autonomous-context-fixtures.ts` (8.2.0) fixture 20 still `FAIL`s, now listing one additional offender on top of the ones already documented in every prior 8.2.x entry — `lib/ai/autonomousLearning/contracts.ts` (a legitimate type-only import of `DecisionSource`, re-exported the same way every other 8.2.x `contracts.ts` already does).

### Authority boundaries — confirmed unmodified this pass

Phase 7 Oracle (`lib/ai/oracle/grading.ts` and all canonical analysis), Phase 8.2.6 (`lib/ai/autonomousDecision/*`), Phase 8.2.7 (`lib/ai/autonomousExecution/*`), and every 8.1.x learning authority (`lib/ai/decisionOutcome/*`, `lib/ai/decisionEvaluation/*`, `lib/ai/failurePatterns/*`, `lib/ai/decisionMemory/*`, `lib/ai/adaptiveConstraint/*`, `lib/ai/learningValidation/*`, `lib/ai/decisionLearning/lifecycle.ts`) are byte-for-byte untouched — confirmed by `diff` against the original zip extraction for every file this pass could have plausibly touched. `lib/elvoid/paperTrader.ts` and `lib/ai/oracle/execute.ts` are likewise untouched (their pre-existing wiring is only read/asserted-on by fixture 14, never called or modified by `lifecycle.ts`).

### Known limitations, honestly reported

- No fake/mock scenario was needed to "prove" this loop closes, because it was already closed by pre-existing code — this pass's fixtures assert against `AutonomousPaperExecutionResult` shapes directly (the exact contract Phase 8.2.7 already produces) rather than spinning up a live Supabase/Learning DB integration test, matching every prior 8.x phase's offline-fixture convention.
- `classifyAutonomousLearningLifecycle()` currently has zero callers anywhere in the app — it is offered as infrastructure for a future monitoring/dashboard consumer, not wired into any route/cron/UI, matching the task's own "no routes/UI/cron unless absolutely required" boundary (nothing required it).
- Failure Pattern / Adaptive Constraint / Constraint Validation remain fully unwired from any automatic trigger — recomputing them (on a schedule, or any other cadence) is explicitly deferred, per this task's own strategic note, to a future, separately-approved phase.
- Phase 8.2.7 itself was found already-implemented in this codebase snapshot with no corresponding `CHANGES.md` entry; this pass added a short retroactive note above so the record is accurate, but did not re-verify or re-test 8.2.7's own implementation beyond re-running its existing fixture script (which the sandbox's missing `@supabase/supabase-js` package prevented from executing — see above).

### Phase status

- Phase 8.1.0 – 8.1.5: **COMPLETE** (unchanged this pass).
- Phase 8.2.0 – 8.2.6: **COMPLETE** (unchanged this pass).
- Phase 8.2.7: Autonomous Paper Execution Adapter — **COMPLETE** (found pre-existing this pass; documented retroactively above; source unchanged).
- Phase 8.2.8: Closed Learning Feedback Loop — **COMPLETE**. Finding: the requested loop (EXECUTE → Paper Trade → Trade Outcome → Decision Experience → Decision Evaluation) was already fully wired by pre-existing Phase 8.1.0/8.1.1.1 code reached transitively through Phase 8.2.7 — zero protected-file changes were required. This pass adds one small, pure, unwired classification module (`lib/ai/autonomousLearning/*`) documenting and exposing that finding for future consumers, plus its fixture suite. Failure Pattern → Adaptive Constraint → Constraint Validation remain intentionally un-auto-triggered recompute stages, per this task's own explicit instruction.
- Explicitly NOT implemented this pass (per task instruction): Phase 8.2.9 (optimization/monitoring) and any automatic/scheduled trigger for `recomputeFailurePatterns()`/`recomputeAdaptiveConstraints()`/`recomputeConstraintValidations()` — both out of scope until separately approved.

## Phase 8.2.9 — Final Autonomous Activation & Closed Learning Loop

**Goal**: ELVOID Pro no longer needs a click on "Execute Signal" to produce a Paper Trade. The Phase 7 → 8.2.8 pipeline (already fully built by prior phases, confirmed unmodified below) now runs on a runtime trigger, sequences automatically end-to-end, protects against duplicate execution across repeated cycles, and closes the learning loop by refreshing Failure Patterns → Adaptive Constraints → Constraint Validations after trades close — so the *next* autonomous decision can actually see validated learning from *previous* ones.

### What was built (orchestration only — see "Authority boundaries" below)

- **`lib/ai/autonomousRuntime/orchestrator.ts`** — `runAutonomousCycle(symbol, interval, calendar, news)`. Sequences, per symbol, per cycle: `assembleOracleContext` → `computeConfluence` → `buildOracleRiskPlan` → `gradeConfluence` (Phase 7, unchanged) → the full Phase 7.2–8.0.5 context layer (`mtf`, `regime`, `liquidityOrderFlow`, `scenarios`, `contradictions`, `arbitration`, `riskIntelligence`, `cognitiveObservation`, `workingMemory`, `hypotheses`, `cognitiveConflict`, `decisionContext`, `learningContext` — each individually try/caught to `null`, mirroring `app/api/elvoid-pro/oracle/route.ts`'s own established defensive composition rather than duplicating its logic under a new name) → `queryDecisionMemory` (8.1.3) → `getConstraintValidations` (8.1.5, new read — see below) → `buildAutonomousDecisionContext` (8.2.0) → `qualifyAutonomousDecision` (8.2.2) → `analyzeMacroIntelligence` (8.2.3) → `analyzeEventImpact` (8.2.4) → `validatePreEntry` (8.2.5) → `decideAutonomous` (8.2.6) → **duplicate-execution gate** (new, orchestration-only, see below) → `executeAutonomousPaperTrade` (8.2.7 — persists the decision trace internally for every outcome) → `classifyAutonomousLearningLifecycle` (8.2.8). Never throws; a failure building the canonical assessment resolves the cycle to `stage: "NO_ASSESSMENT"` rather than fabricating a WAIT/REJECT for a symbol the Oracle never actually assessed.
- **`lib/ai/autonomousRuntime/batch.ts`** — `runAutonomousBatch(interval)`. Fetches the economic calendar (`getEconomicCalendar()`) and news feed (`getNews()`) **once** per batch and shares them across every symbol's cycle (both are pure over whatever `asOf`/calendar/news they're given, so this changes nothing about their output). Iterates the user's **existing, live, editable watchlist** — `lib/elvoid/watchlist.ts::getWatchlistCoins()`, the exact same list "Scan Market" already scans — sequentially, one `runAutonomousCycle()` per symbol, each independently isolated so one symbol's failure can never abort the rest of the batch.
- **`lib/ai/autonomousRuntime/lock.ts`** — a plain, single-row-per-id advisory lock (`autonomous_runtime_lock` table, new — see migration below), claimed atomically via a conditional `UPDATE ... WHERE running = false`, with a 10-minute stale-reclaim so a crashed invocation self-heals within one tick rather than deadlocking the runtime forever. Fails **open** (proceeds unlocked) if the Learning DB isn't configured — a lock that can't be verified is never treated as "block everything".
- **`lib/ai/autonomousRuntime/dedup.ts`** — Phase 8.2.9 §6's duplicate-execution protection. `buildAutonomousSetupIdentity()` is a **deterministic, pure** function of `symbol + side + grade + invalidation` (deliberately excluding `timestamp`, which always advances every cycle, and `confidence`, which can drift a point or two without representing a new setup). This is a **second, coarser** identity layered *above* `lib/ai/oracle/execute.ts::buildOracleSignalId()` — that function's own per-assessment idempotency key (and its UNIQUE DB index) is completely untouched and remains the execution-layer's own protection; this module exists because that key changes every cycle even when the underlying setup hasn't, which is exactly what a repeated autonomous cycle needs guarding against. `autonomous_execution_dedup` (new table, one row per `(source, symbol)`) records the last-executed setup identity; `isDuplicateSetup()` is the sole predicate the orchestrator's gate consults.
- **The dedup gate itself, in `orchestrator.ts`**: when `decideAutonomous()`'s own pure answer is `EXECUTE` **and** the candidate setup identity matches the last-executed one for that symbol, the orchestrator downgrades the **action taken this cycle** to `WAIT` (`executeAutonomousPaperTrade` is called with `decision: "WAIT"`, so no Paper Trade is created and no second `executeOracleSignal()` call happens) and records `dedupApplied: true` on the returned `AutonomousCycleResult`. This is **not** a second decision engine — `decideAutonomous()`'s own signals/qualification/macro/eventImpact/preEntry reasoning is never recomputed or second-guessed; only the execution action is gated. WAIT and REJECT decisions never consult dedup at all — only a candidate EXECUTE can be downgraded. A genuinely changed setup (side/grade/invalidation actually different) always executes normally and may be reconsidered.
- **`lib/ai/autonomousRuntime/learningRefresh.ts`** — Phase 8.2.9 §7-8's closing piece. `runLearningRefresh()` sequences the three **already-built, already-callable** recompute functions in the required order — `recomputeFailurePatterns()` (8.1.2) → `recomputeAdaptiveConstraints()` (8.1.4) → `recomputeConstraintValidations()` (8.1.5) — introducing **zero** new pattern-detection/constraint-generation/validation logic, guarded by the shared lock so overlapping triggers can never run the sequence concurrently or duplicate work. A failure at any step stops that run but never deletes/invalidates the previous successful snapshot (each recompute function is its own full recompute-and-upsert). `triggerLearningRefreshBestEffort()` is the fire-and-forget wrapper.
- **One line added to `lib/elvoid/paperTrader.ts::writeClose()`**: `completeDecisionLearningLifecycle(signal.id)` (pre-existing, Phase 8.1.1.1) now has a `.then(() => triggerLearningRefreshBestEffort())` added before its existing `.catch(...)`. **Discovery this pass**: outcome capture → Decision Evaluation was *already* fully automatic on every trade close — Phase 8.2.8's CHANGES.md entry documented this correctly. What was still missing was the **aggregate** refresh (Failure Patterns → Adaptive Constraints → Constraint Validations), which is intentionally *not* a per-evaluation operation (would be wasteful to fully recompute on every single trade) — this one line closes that gap with a locked, debounced-by-the-lock trigger, the smallest possible change to a protected call-site.
- **`lib/ai/learningValidation/repository.ts::getConstraintValidations(source)`** — new **read-only** function, added because Phase 8.2.9 §8's consumer boundary (*"the next `AutonomousDecisionContext` must see refreshed `constraint_validations` WHERE `status = VALID`, never read `adaptive_constraints` directly"*) had no reader to satisfy it with — the file was write-only before this pass (`persistConstraintValidations`/`recomputeConstraintValidations`). Returns every status (not pre-filtered to VALID) straight off `constraint_validations`; `lib/ai/autonomous/context.ts::buildAutonomousDecisionContext()` (unchanged, Phase 8.2.0) is still the sole place that narrows to `status === "VALID"` via its own existing `filterValidConstraints()`. Confirmed by grep: `lib/ai/autonomous/context.ts` contains zero references to `adaptive_constraints` — the consumer boundary is intact.
- **Runtime trigger (§5)** — two triggers, matching this exact repo's own established convention (`app/api/binance/auto-trade/tick`, `app/api/whale/indexer/run` — both already use this identical GET-cron-secret + POST-client-ping pattern):
  - **`app/api/elvoid-pro/autonomous/tick/route.ts`** (`GET`, cron-secret gated via `CRON_SECRET`; `POST`, open) — calls `runAutonomousBatch()`. `maxDuration = 60`.
  - **`lib/hooks/useAutonomousRuntimeTick.ts`** — client hook, `POST`s the tick route every 60s while mounted. Wired into `OraclePanel` (see UI below), matching `useWhaleIndexerTick`/`useBinanceTrading`'s exact pattern — **this is what makes autonomous analysis run out of the box on any hosting plan**, no cron configuration required, as long as ELVOID Pro is open in at least one tab anywhere.
  - **`vercel.json`** — added `{ "path": "/api/elvoid-pro/autonomous/tick", "schedule": "0 21 * * *" }`, a **daily** cron, deliberately matching this repo's three pre-existing cron entries (`0 19`, `30 19`, `0 20 * * *`) which are *also all daily* — a strong signal this deployment is on Vercel's **Hobby** plan, which only permits a once-per-day cron schedule and will reject/coerce anything more frequent. **This is an honestly conservative choice, not the primary trigger** — real intraday coverage comes from the client-side tick hook above. If this project is actually on a Pro-or-higher plan, `vercel.json`'s schedule can safely be tightened (e.g. `*/15 * * * *`) for true zero-open-tab unattended operation; `runAutonomousBatch()`'s own lock already makes any schedule safe to raise without code changes.
- **`app/api/elvoid-pro/autonomous/status/route.ts`** — pure **read** endpoint (`GET ?symbol=BTC`), membership-gated like every other ELVOID Pro route. Returns the most recent decision trace already persisted for that symbol plus whether any `VALID` constraint validation currently exists for `ELVOID_PRO_ORACLE`. Triggers nothing — never calls `runAutonomousCycle`/`runAutonomousBatch` — so polling it can never itself cause a Paper Trade.
- **UI (§9)** — `components/elvoid-pro/AISignal/OraclePanel.tsx`: the manual **"Execute Signal" button, its `executeState`/`executeMsg` state, and its call to `POST /api/elvoid-pro/execute-signal` are removed** (that route itself is left in place, untouched, in case anything else still references it — nothing in the app does after this change, confirmed by grep). Replaced with `<AutonomousStatusCard symbol={symbol} />`, an **observation-only** panel showing "AUTONOMOUS MODE ● MONITORING", the latest decision (EXECUTE/WAIT/REJECT), execution outcome ("PAPER TRADE CREATED" / "no trade"), and a "VALIDATED LEARNING ACTIVE" indicator — polling the new status route every 20s and mounting `useAutonomousRuntimeTick(true)` so the runtime advances whenever this panel is on screen. The user views; the user does not authorize.

### Duplicate execution — exactly what counts as a duplicate decision (§6, documented per instruction)

Two autonomous cycles for the **same `(source, symbol)`** whose canonical read produces the **same `side` + `grade` + `invalidation` text** are the same setup, regardless of how many cycles apart or how much `confidence`/`timestamp` moved in between. The **first** EXECUTE for a setup always goes through. Any **later** cycle whose candidate setup identity still matches is safely downgraded to WAIT before `executeAutonomousPaperTrade()` is ever called a second time for it — zero duplicate Paper Trades, confirmed by fixture.

### Learning refresh ordering & isolation (§7, confirmed)

`recomputeFailurePatterns()` → `recomputeAdaptiveConstraints()` → `recomputeConstraintValidations()`, strictly in that order, inside a single lock claim; a mid-sequence failure stops that run without corrupting or deleting the previous successful snapshot; overlapping triggers (two trades closing in quick succession) safely collapse to one refresh via the lock's `already_running` outcome.

### PAPER TRADE ONLY — confirmed (§13)

Static scan of every new file this phase added (`lib/ai/autonomousRuntime/*`, `app/api/elvoid-pro/autonomous/*`, `lib/hooks/useAutonomousRuntimeTick.ts`) for `placeOrder`/`createOrder`/exchange order calls/`signTransaction`/private-key handling/live-trading references: **zero matches**. `executeAutonomousPaperTrade()` (8.2.7, unmodified) remains the sole execution path, and it is itself Paper-only, unchanged.

### Testing (§14)

`scripts/phase8/autonomous-runtime-fixtures.ts` — 15/15 passed. Covers `buildAutonomousSetupIdentity()` determinism (stable across timestamp advancing and confidence drift; changes on side/grade/invalidation change) and the dedup gate decision table (cycle 1 EXECUTE-through, cycle 2 EXECUTE-on-unchanged-setup → WAIT+`dedupApplied`, cycle 2 EXECUTE-on-changed-setup → EXECUTE, WAIT/REJECT never consult dedup, defensive null-canonical-snapshot never throws).

**Known, honestly-reported limitation**: `runAutonomousCycle()` itself has **no dependency-injection seam** (unlike `executeAutonomousPaperTrade`, which already has `AutonomousExecutionDeps` for exactly this reason) — it calls `assembleOracleContext`/Binance/Learning-DB-backed functions directly. True end-to-end orchestrator fixture testing (the "full qualified context → EXECUTE path", "CAUTION → WAIT", "BLOCKED → REJECT", "missing context → WAIT" cases §14 asks for at the *pipeline* level) was **not** built this pass beyond what the sub-phase fixtures already cover individually (8.2.2 through 8.2.7 each already fixture-test their own qualify/macro/eventImpact/preEntry/decide/execute step in isolation) plus this pass's own dedup-gate coverage above. Closing this gap cleanly needs either a live Learning DB/Binance sandbox or a follow-up refactor giving `runAutonomousCycle()` its own injectable-deps seam — flagged here rather than skipped silently.

### Regression — full suite re-run this pass, dependencies installed successfully

Unlike the Phase 8.2.8 pass (blocked by a missing `@supabase/supabase-js` install), this pass ran `npm install` successfully (624 packages) and could exercise the **entire** project, not just isolated fixture scripts:

- **`npx tsc --noEmit` across the full project: 0 errors.** This is the strongest available regression signal in this sandbox (no live Supabase/Binance credentials exist here to run the app itself) — every new file type-checks against every existing contract it consumes, and no existing file was left in a broken state.
- **Every `scripts/phase8/*.ts` fixture script re-run directly, including ones the previous pass could not execute**: `adaptive-constraint-fixtures.ts` (21/21), `autonomous-decision-fixtures.ts` (25/25), `autonomous-paper-execution-fixtures.ts` (22/22 — previously blocked, now runs clean), `cognitive-*-fixtures.ts` (all 5, all pass), `decision-evaluation-fixtures.ts` (36/36), `decision-memory-fixtures.ts` (20/20), `decision-qualification-fixtures.ts` (23/23), `decision-trace-fixtures.ts` (20/20), `event-impact-fixtures.ts` (51/51), `failure-pattern-fixtures.ts` (16/16), `learning-db-env-fixtures.ts` (all pass), `macro-intelligence-fixtures.ts` (43/43), `pre-entry-validation-fixtures.ts` (25/25) — **zero new regressions** in any of these.
- **Known pre-existing failures, now with one new (expected, intentional) offender each — not new regressions**, all caused by this phase's *deliberate* wiring, exactly what §17's acceptance criteria require:
  - `autonomous-context-fixtures.ts` fixture 20 ("zero external call sites into `lib/ai/autonomous/*`") — was already documented FAILing in every prior 8.2.x entry (that module has been progressively wired up phase by phase); now additionally lists `lib/ai/autonomousRuntime/*` and `app/api/elvoid-pro/autonomous/tick/route.ts` as callers, exactly the wiring this phase's own §17 checklist requires ("User does not need to click Execute", "Oracle analysis enters the autonomous pipeline automatically").
  - `autonomous-learning-fixtures.ts` fixture 14a, `decision-learning-lifecycle-fixtures.ts` fixtures 7-8, and `decision-outcome-fixtures.ts` fixtures 30/32 — all four are static regexes asserting `writeClose()`'s exact literal call shape `completeDecisionLearningLifecycle(signal.id).catch(...)` with no `.then(`. This phase intentionally changed that one call site to `completeDecisionLearningLifecycle(signal.id).then(() => triggerLearningRefreshBestEffort()).catch(...)` — still fire-and-forget (never `await`ed), still has a `.catch()`, the `ai_signals` status update below it is still a separate unconditional statement — exactly what every one of those four fixtures otherwise still confirms; only the literal-`.then()`-absence assertion itself is now stale. This is precisely the §7/§8 requirement ("after a trade closes... must eventually refresh the derived learning state").
  - `learning-validation-fixtures.ts` fixture 19 ("repository.ts reads only `adaptive_constraints`, writes only `constraint_validations`") — now also lists `constraint_validations` as a read target, because this phase added `getConstraintValidations()` (the §8 consumer-boundary reader documented above). Intentional; the write side is completely unchanged.
  
  Per this task's own instruction, **none of these four pre-existing fixture files were edited** to make the run green — the assertions are correctly stale against source they were never designed to anticipate, and are reported here rather than silently patched.

### Authority boundaries — confirmed unmodified this pass

Every file section 12 lists as protected — Oracle grading/confluence/risk/side/confidence (`lib/ai/oracle/*`), the AI Signal engine, Paper Trade accounting logic (`lib/elvoid/paperTrader.ts`'s trade-execution/close mechanics — only the one documented line was added), Decision Evaluation semantics, Failure Pattern qualification thresholds, Adaptive Constraint generation rules, and Learning Validation rules — is **byte-for-byte unchanged except for the two additive, non-destructive changes documented above** (`paperTrader.ts`'s one added `.then()`, `learningValidation/repository.ts`'s one added read-only function). `lib/ai/autonomousDecision/*`, `lib/ai/autonomousExecution/*`, `lib/ai/decisionQualification/*`, `lib/ai/macroIntelligence/*`, `lib/ai/eventImpact/*`, `lib/ai/preEntryValidation/*`, `lib/ai/autonomousLearning/*`, and `lib/ai/decisionTrace/*` are **completely untouched** — this phase only ever calls their existing exported functions, never edits them.

### Final acceptance criteria (§17) — status

- [x] User does not need to click Execute in ELVOID Pro — button removed, replaced with `AutonomousStatusCard`.
- [x] Oracle analysis enters the autonomous pipeline automatically — `runAutonomousCycle()`.
- [x] Qualification / Macro / Event Impact / Pre-Entry run automatically — same function.
- [x] AI autonomously chooses EXECUTE / WAIT / REJECT — `decideAutonomous()`, unmodified.
- [x] EXECUTE automatically creates an ELVOID Pro Paper Trade — `executeAutonomousPaperTrade()`, unmodified, now called by the runtime instead of a button handler.
- [x] WAIT / REJECT create no Paper Trade — unchanged 8.2.7 behavior, confirmed by 8.2.7's own fixture suite (still 22/22).
- [x] Duplicate autonomous cycles cannot duplicate the same Paper Trade — new dedup gate, fixture-confirmed.
- [x] Closed Paper Trades enter the existing learning lifecycle — confirmed pre-existing, unchanged.
- [x] Decision Evaluation is produced automatically — confirmed pre-existing, unchanged.
- [x] Failure Patterns / Adaptive Constraints / Constraint Validations can be refreshed automatically/safely — new `learningRefresh.ts`, locked, ordered.
- [x] Future autonomous decisions consume only VALID learning constraints — `getConstraintValidations()` (new read) → `buildAutonomousDecisionContext()`'s existing `filterValidConstraints()` (unchanged) → `adaptive_constraints` never read directly by the consumer, confirmed by grep.
- [x] User can observe autonomous activity without manually authorizing execution — `AutonomousStatusCard` + status route.
- [x] No live trading capability introduced — confirmed by static scan, zero matches.
- [x] Existing Oracle authority remains canonical — confirmed unmodified.
- [~] Full regression suite has no new regressions — **true for every functional fixture**; four pre-existing *static-scan* assertions are now expected-stale for the reasons documented above (they assert the absence of wiring this phase was explicitly asked to add). Not treated as passing silently — reported here in full, source of those four fixture files left untouched per instruction.

### Known limitations, honestly reported

- Orchestrator-level end-to-end fixture testing (the full pipeline as one unit, across EXECUTE/WAIT/REJECT/insufficient-context paths) needs a dependency-injection seam `runAutonomousCycle()` doesn't have yet, or a live test sandbox — see "Testing" above.
- The Vercel Cron entry is deliberately conservative (once daily) pending confirmation of the actual hosting plan tier; the client-side tick hook is the practical primary trigger until that's confirmed.
- The four pre-existing static-scan fixture assertions documented above are now stale by design; they were left unedited per this task's own instruction and should be updated in a future pass once a maintainer explicitly reviews and re-approves the new call-site shapes they check.
- `app/api/elvoid-pro/execute-signal/route.ts` (the old manual-execute endpoint) was left in place, unused, rather than deleted — nothing in the app calls it after this pass (confirmed by grep), but removing dead routes outright felt like a decision worth flagging rather than making silently.
