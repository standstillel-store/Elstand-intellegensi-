...
export function IndicatorSelector({
  ...
  const primaryGroups = CHART_MODE_GROUPS.filter((g) => g.primary);
  const secondaryGroups = CHART_MODE_GROUPS.filter((g) => !g.primary);
  // If the active mode lives in a secondary group, keep that section expanded
  // so the current selection is never hidden behind the "More tools" toggle.
  const activeInSecondary = secondaryGroups.some((g) => g.items.some((i) => i.id === activeMode));
      ...
      {open && (
        <div className="absolute left-0 top-[calc(100%+6px)] z-30 max-h-[70vh] w-64 overflow-y-auto rounded-lg border border-line bg-bg-raised shadow-2xl">
          ...
          <button
            ...
            className="flex w-full items-center justify-between px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] text-ink-faint transition-colors hover:text-ink-muted"
          >
            More tools
            <ChevronDown size={12} className={clsx("transition-transform", (showMore || activeInSecondary) && "rotate-180")} />
          </button>

          {(showMore || activeInSecondary) &&
