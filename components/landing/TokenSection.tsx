...
function UtilityCard({
  ...
  return (
    <div className="landing-glass relative rounded-xl p-5">
      {status === "soon" && (
        <span className="eyebrow absolute right-4 top-4 text-[8px] tracking-[0.15em] text-landing-ink-faint">COMING SOON</span>
      )}
      {status === "live" && <span className="live-dot absolute right-4 top-4" aria-hidden="true" />}
      <h3 className="font-display text-base text-landing-ink">{title}</h3>
      <p className="mt-2 text-[13px] leading-relaxed text-landing-ink-muted">{description}</p>
    </div>
  );
}
