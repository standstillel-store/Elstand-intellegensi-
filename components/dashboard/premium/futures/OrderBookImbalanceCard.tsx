...
import type { OrderBookDepthData } from "@/lib/intelligence/premiumMicrostructure";
...
export function OrderBookImbalanceCard({ pair, book }: { pair: string; book: OrderBookDepthData }) {
  return (
    <section className="panel flex flex-col gap-3 p-4">
      <div className="flex items-center justify-between">
        ...
        {book.midPrice !== undefined && (
          <span className="text-[11px] text-ink-faint">Mid ${book.midPrice.toLocaleString()}</span>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div>
          <div className="text-[10px] uppercase tracking-wide text-ink-faint">Bid Dominance</div>
          <div className="text-[15px] font-semibold text-up">
            {book.bidDominancePercent !== undefined ? `${book.bidDominancePercent.toFixed(1)}%` : "N/A"}
          </div>
        </div>
        <div>
        ...
        <div>
          <div className="text-[10px] uppercase tracking-wide text-ink-faint">Ask Dominance</div>
          <div className="text-[15px] font-semibold text-down">
            {book.askDominancePercent !== undefined ? `${book.askDominancePercent.toFixed(1)}%` : "N/A"}
          </div>
        </div>
      </div>
