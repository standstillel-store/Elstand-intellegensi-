                    ...
                    return (
                      <div key={a.symbol} className="flex items-center justify-between gap-2">
                        ...
                        <div className="text-right">
                          <p className="mono-num text-[12px] font-semibold text-ink [filter:drop-shadow(0_0_3px_currentColor)]">{formatPrice(a.price)}</p>
...
function AssetIcon({ asset, size = 24 }: { asset: WatchlistAsset; size?: number }) {
  ...
  return (
    <span
      className="relative flex shrink-0 items-center justify-center overflow-hidden rounded-full bg-bg-raised text-[9px] font-bold text-ink-faint ring-1 ring-line"
      style={{ width: size, height: size }}
    >
      {asset.symbol.slice(0, 1)}
      {asset.icon && (
        // eslint-disable-next-line @next/next/no-img-element -- external CoinGecko logos, next/image would need remote-pattern config
        <img
          src={asset.icon}
          alt=""
          loading="lazy"
...
function WatchlistCard({ asset }: { asset: WatchlistAsset }) {
  ...
  return (
    <div className={`w-[112px] shrink-0 rounded-lg border bg-bg-raised/40 px-3 py-2.5 ${up ? "border-up/20" : "border-down/20"}`}>
      <div className="flex items-center gap-1.5">
        <AssetIcon asset={asset} size={18} />
        <p className="truncate text-[10px] font-semibold text-ink">{asset.symbol}</p>
      </div>
      <p className="mono-num mt-1.5 text-[13px] font-semibold text-ink [filter:drop-shadow(0_0_3px_currentColor)]">{formatPrice(asset.price)}</p>
...
// Tailwind scans source for literal class strings, so tone classes must be
// spelled out in full here rather than built with template interpolation —
// `border-${tone}/25` would never be picked up by the compiler.
const TONE_CLASSES: Record<string, { border: string; text: string }> = {
  signal: { border: "border-signal/25", text: "text-signal" },
  down: { border: "border-down/25", text: "text-down" },
  up: { border: "border-up/25", text: "text-up" },
  gold: { border: "border-gold/25", text: "text-gold" },
  amber: { border: "border-amber/25", text: "text-amber" },
};

function toneFromIconBg(iconBg: string): { border: string; text: string } {
  const match = iconBg.match(/text-([a-z]+)/);
  const key = match?.[1];
  return (key && TONE_CLASSES[key]) || { border: "border-line/60", text: "text-ink" };
}
...
function SummaryCard({
  ...
  const tone = toneFromIconBg(iconBg);
  return (
    <div className={`flex items-start gap-2.5 rounded-lg border bg-bg-raised/40 px-3 py-2.5 ${tone.border}`}>
      ...
      <div className="min-w-0">
        ...
        <p className={`mt-0.5 truncate text-[15px] font-bold tracking-tight [filter:drop-shadow(0_0_4px_currentColor)] ${valueCls ?? "text-ink"}`}>
          {value}
        </p>
...
function InsightCard({ icon: Icon, iconBg, label, value, sub }: { icon: LucideIcon; iconBg: string; label: string; value: string; sub: string }) {
  const tone = toneFromIconBg(iconBg);
  return (
    <div className={`rounded-lg border bg-bg-raised/40 px-3 py-2.5 ${tone.border}`}>
      ...
      <p className={`mt-0.5 text-sm font-bold tracking-tight [filter:drop-shadow(0_0_4px_currentColor)] ${tone.text}`}>{value}</p>
